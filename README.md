...
* **Spectrum Estimation:** Before any DCT data retrieval, the algorithm analyzes multiple spatially separated 512x512 zero-mean windows, applies a **2D Separable Hann Window** to eliminate rectangular boundary leakage, and performs a **2D Fast Fourier Transform (FFT)**. Each window proposes a rotation and scale estimate; compatible proposals are clustered into consensus candidates, preventing a localized high-contrast edit from unilaterally forcing a destructive full-frame correction. A lossless identity candidate is always retained for downstream validation.
* **Sub-pixel Parameter Recovery:** Within each analysis window, peak positions are refined via a **Grandke Interpolator**. The system eliminates discrete bin bias and derives precise rotation and scaling parameters before the candidate set is validated by the DCT synchronization and FEC pipeline.
...
* **Other Crop Geometries (Theoretical Extension):** The right-edge sweep above is a **single-variable** stress test that maximizes one-dimensional redundancy (full 3024px height intact). For other geometries not yet swept in this report, the expected behavior is as follows:
  * **Opposite-edge / bilateral crops** (e.g., left + right, top + bottom): The **32-bit sync header** and **64-offset sliding-window scan** are designed to re-lock grid phase after translation; survival limits should remain in the same ballpark as a single-edge crop at an equivalent net shrink along that axis—typically only a few percentage points lower, not a qualitatively different regime.
  * **Adjacent-edge / corner crops** (e.g., top-right): The same alignment machinery applies, but the hard limit is governed by **both** axes. If the same per-edge crop ratio is applied to two adjacent sides, the **shorter remaining dimension** hits the $136\text{ px}$ tile period first (e.g., $3024 \times 4\% = 121\text{ px} < 136\text{ px}$ at 96% per edge), so the failure cliff can arrive earlier than a right-only crop at the same nominal percentage—not because sync search is weaker, but because 2D spatial continuity is broken on the short side.
...
* **Geometric Isolation:** Because the generated image retains the exact source width and height, any extraction change can be attributed to local spatial corruption rather than an unintended resize outside the measured **0.85x–1.50x** scale envelope.
...
* **Regression Coverage:** The DEBUG attack suite includes a full-resolution diagonal-scribble case and a compound downscale-plus-scribble guard case. The former verifies correct recovery under pure local damage; the latter verifies safe rejection rather than false success when the geometric attack is outside the supported scale range.
